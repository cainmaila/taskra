...
| **我的任務** (My Tasks) | 列出無專案任務；頂部 member picker 切換成員，預設顯示全部；auth 後預設選登入者，仍可切換看他人任務 |
...
| 無專案任務            | 只出現在「我的任務」，member picker 切換誰的任務 | 個人待辦與專案任務明確分離；無 auth 仍可切換成員視角 |
...
「我的任務」頂部有 member picker，可切換查看任何成員的無專案任務。

| 階段         | 行為                                                         |
| ------------ | ------------------------------------------------------------ |
| MVP 無 auth  | picker 預設「全部」，可選任一成員；選擇持久化至 localStorage |
| 加入 auth 後 | picker 預設登入者，仍可切換看他人任務                        |

因為 assignee 必填，所有無專案任務必然有負責人。

## Parent Task Picker 需求

### 問題

Task 視窗的 Parent Task 欄位目前需手動輸入 task ID，操作不友善。
...
- 建立新 task 時設定 parent
- 事後編輯 task 重新歸類（兩種場景同等頻率）

**搜尋行為**

- 主要依 **title 關鍵字** 模糊搜尋
- 可選過濾維度：**Project**、**Tags**
- 搜尋結果顯示：**Title + Project + Tags**（幫助辨識正確 task）

**選取後顯示**

- 欄位顯示 task **title**（可點擊查看該 task）
- 儲存時寫入 `parent_task_id`
...
| Decision       | Choice                 | Rationale            |
| -------------- | ---------------------- | -------------------- |
| 搜尋主要維度   | Title 關鍵字           | 使用者最常記得 title |
| 結果顯示欄位   | Title + Project + Tags | 足以辨識，不過多     |
| 選取後欄位顯示 | Title（可點擊）        | 人讀友善，點擊可跳轉 |
